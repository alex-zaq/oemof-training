...
Bel_NPP_fix = tech_fix[Bel_NPP_fix_lst[0]]
NEW_NPP_Toi = tech_fix[NEW_NPP_TOI_fix_lst[0]]
# G_Turb_fix = tech_fix['G_Turb']
Wind_fix = tech_fix["Wind"]
Solar_fix = tech_fix["PV"]
Hydro_fix = tech_fix["Hydro"]
...
B_Block_Station = solph.Transformer(
        label = "B_Block_Station",
        inputs = {b_gas:solph.Flow()},
        outputs = {b_el:solph.Flow(nominal_value=669.512,fix = tech_fix["Block_Stations"] ,variable_costs=1)},
        conversion_factors = {b_el:0.8},
    )
energysystem.add(B_Block_Station)

C_VER = solph.Source(
        label = "C_VER",
        # inputs = {b_gas:solph.Flow()},
        outputs = {b_el:solph.Flow(nominal_value=50.608,fix = tech_fix["Block_Stations"] ,variable_costs=1)},
        # conversion_factors = {b_el:1},
    )
energysystem.add(C_VER)

# fix=tech_fix["CHP_Steam"]
D_CHP_Steam = solph.Source(
    label="D_CHP_Steam",
    inputs = {b_gas:solph.Flow()},
        outputs = {b_el:solph.Flow(nominal_value=300,variable_costs = 1), b_steam:solph.Flow()},
        conversion_factors = {b_el:0.164, b_steam:0.656},
    )
...
E_CHP_Heat_Water = solph.Transformer(
        ...
        conversion_factors = {b_el:0.27, b_heat:0.58},
    )
...
F_Wind = solph.Source(
        label = "F_Wind",
        # inputs = {b_gas:solph.Flow()},
        outputs = {b_el:solph.Flow(nominal_value=104.24,fix = Wind_fix ,variable_costs=0)},
        # conversion_factors = {b_el:1},
    )
energysystem.add(F_Wind)


G_Solar = solph.Source(
        label = "G_Solar",
        # inputs = {b_gas:solph.Flow()},
        outputs = {b_el:solph.Flow(nominal_value=163.44,fix = Solar_fix ,variable_costs=0)},
        # conversion_factors = {b_el:1},
    )
energysystem.add(G_Solar)


H_Hydro = solph.Source(
        label = "H_Hydro",
        # inputs = {b_gas:solph.Flow()},
        outputs = {b_el:solph.Flow(nominal_value=95,fix = Hydro_fix ,variable_costs=0)},
        # conversion_factors = {b_el:1},
    )
energysystem.add(H_Hydro)


I_Turb_K = solph.Transformer(
        label = "I_Turb_K",
        inputs = {b_gas:solph.Flow()},
        outputs = {b_el:solph.Flow(nominal_value=3443 , variable_costs=500)},
        conversion_factors = {b_el:0.45},
    )
energysystem.add(I_Turb_K)
...
J_CCGT = solph.Transformer(
        label = "J_CCGT",
        inputs = {b_gas:solph.Flow()},
        outputs = {b_el:solph.Flow(nominal_value=1261.1, min= 0.25,variable_costs=1)},
        conversion_factors = {b_el:0.60},
    )
energysystem.add(J_CCGT)
...
El_demand =  solph.Sink(
        label="El_demand",
        inputs = {b_el: solph.Flow(fix =demands_power["Electricity"], nominal_value = 6205.69 )} 
...
\
Steam_demand = solph.Sink(
        label = "Heat_water_demand",
        inputs = {b_heat:solph.Flow(fix = demands_power["Heat_Water"], nominal_value = 300  )}
    
    )
energysystem.add(Steam_demand)
...
gas_sum = sum(results[(Import_Gas, b_gas )]['sequences']['flow'])
# ccgt = sum(results[(F_CCGT, b_el )]['sequences']['flow'])


# print(ccgt)

gas_sum = (gas_sum/1000000)*0.108;
# print(gas_sum)
...
custom_title = 'Потребление газа:'+str(round(gas_sum,3)) +' млрд м3'

ax = data.plot(kind="area",  stacked = True , grid=True, rot=0 , title = custom_title)

...
electricity_chp_bus = solph.Bus(label = "electricity_chp_bus" )
energysystem.add(electricity_chp_bus)
     
 
chp_electricity_tr_first = solph.components.Transformer (
    label = 'chp_electricity_tr_first',
		inputs = {natural_gas_global_bus: solph.Flow()},
		outputs = {electricity_chp_bus: solph.Flow(nominal_value = 1000, variable_costs = -1000),
             	heat_water_bus: solph.Flow(variable_costs = -1000)},
		conversion_factors = {natural_gas_global_bus:4,  electricity_chp_bus:1, heat_water_bus:2}
		
 ) 
energysystem.add(chp_electricity_tr_first)
   
   
   
    
chp_electricity_tr_second = solph.components.Transformer (
    label = 'chp_electricity_tr_second',
		inputs = {electricity_chp_bus: solph.Flow()},
		outputs = {electricity_bus: solph.Flow(nominal_value = 50, min = 0.1, nonconvex = solph.NonConvex()),
             	},
		conversion_factors = {electricity_chp_bus:1,  electricity_bus:1 }
		
 ) 
energysystem.add(chp_electricity_tr_second)
...
# electr_df['ТЭЦ-электроэнергия'] = electricity_data[((chp_electricity_tr_first.label, electricity_bus.label),'flow')]
# electr_df['Дорогой генератор'] = electricity_data[((back_electricity_tr.label, electricity_bus.label),'flow')]

gas_df = pd.DataFrame()
gas_df['Расход газа ТЭЦ'] = gas_data[((natural_gas_global_bus.label, chp_electricity_tr_first.label),'flow')]

heat_water_df = pd.DataFrame()
heat_water_df['ТЭЦ-горячая вода'] = heat_water_data[((chp_electricity_tr_first.label, heat_water_bus.label),'flow')]

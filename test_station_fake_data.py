...
class Scenario:
    ...
    def calculate(self, year, month):
        ...
        self.set_time_interval(year=year, month=month)
        self.create_oemof_energy_system()    
        self.read_input_excel_data(path='./data_excel', excel_file= 'data_2023.xlsx', sheet_name = month)
        self.processing_input_data()
        self.set_scenario_options()
        self.set_energy_system_with_scenario_options()
        self.launch_solver(solver = 'cplex')
        
        
    def set_time_interval(self, year, month, day_type = 'рабочий'):
        self.hours_count = 24
        self.start_date = find_first_monday(2023, months['февраль'], None)
        
        
    def set_energy_system_with_scenario_options(self):
        pass
        
        
    def create_oemof_energy_system(self):
        date_time_index = pd.date_range(self.start_date, periods = self.hours_count, freq="H")
        self.es = solph.EnergySystem(timeindex = date_time_index, infer_last_interval= True)


    def read_input_excel_data(self, path, excel_file, excel_sheet_name):
        self.excel_reader = get_excel_reader(folder = path, file =  excel_file )
        self.input_data = self.excel_reader(sheet_name = excel_sheet_name)


    def processing_input_data(self, ):
        
        input_data = self.input_data
        
        self.odu_power_abs = input_data['оду']
        self.fixed_load_rel = {
        ...
        self.el_boiler_Belenergo_avail = {
            'январь' : 854/916 ,
            'февраль' : 857/916,
            'март' : 852/916 ,
            'апрель-отоп' : 746/916,
            'апрель-межотоп' : 168/916,
            'май': 149/916 ,
            'июнь': 134/916,
            'июль': 135/916,
            'август':134/916,
            'сентябрь': 114/916,
            'октябрь-межотоп':187/916,
            'октябрь-отоп': 761/916,
            'ноябрь':849/916,
            'декабрь':853/916,
            
        }
        self.el_boilers_hw_groups = {
    ...
    def set_excel_blocks_grouping_options(self):
        pass
    
    
    def set_graph_blocks_grouping_options(self):
        pass
    ...
    def create_el_output_dataframe(self):
        pass
    
        
    def create_hw_output_dataframe(self):
        pass
    
        
        
    def create_steam_output_dataframe(self):
        pass
    ...
    def write_output_excel(self, output_filename):
        pass

    def launch_solver(self, solver):
        pass
    ...
    # excel_reader = get_excel_reader(folder ='./data_excel', file = 'data_2023.xlsx' )
    # input_data = excel_reader(sheet_name= case_info['month'])


    # odu_power_febrary = input_data['оду']
...
##################################################################################################
    ...
    # el_boiler_avail = 1
    el_boiler_avail = el_boiler_Belenergo_avail[case_info['month']]
    scen_builder.add_constraint_for_el_boiler_group(el_boilers_hw_groups['электрокотлы Белэнерго'], 916 * el_boiler_avail)
    ...
    scen_builder.set_natural_gas_price(usd_per_1000_m3 = 159)
    ...
    # scen_builder.remove_large_siemens()
...
##################################################################################################
    ...
    # el_load = result_extractor.get_dataframe_el_boilers_total_consumption('гвс')
...
# import_dataframe_to_excel(results_for_excel_df, './results/', 'февраль-2023.xlsx')
##################################################################################################
##################################################################################################
##################################################################################################
...
if __name__ == "__main__":
    calculate_scenario('февраль')



#  столбец месяца и года станции типов турбин названий станций типов станций, тип энергии
#  последовательный авторасчет по месяцам с общим excel
#  полное потребление газа
#  суммарный уровень электропотребления (две функции)
#  сделать excel файл 2023 в двумя блоками
#  !сделать расчеты для каждой комбинации и перенести в общий excel файл


#  multiperiod version
#  неделя с инвестиционнми затратами


#  (сделано)ограничения для ЭК
#  (сделано)выделить характерные дни в excel
#  (сделано)оставить малые гту
#  (сделано)добавить столбец исходного электрического спроса
#  (сделано)столбей потребления электрокотлов
#  (сделано)добавить столбец - профиль потребления газа млн м3

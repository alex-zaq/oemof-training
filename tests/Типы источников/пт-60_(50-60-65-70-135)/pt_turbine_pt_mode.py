...
from custom_modules.result_proccessing import get_dataframe_by_output_bus_all, get_dataframe_by_input_bus_all, get_dataframe_by_output_bus_single, get_dataframe_by_input_bus_single
...
hw_sink = Generic_sinks(es).create_sink_absolute_demand('гвс_потребитель', hw_bus, demand_absolute_data= hw_load)

counter = Custom_counter()
station_name = 'Тестовая станция'
block_creator = Specific_blocks(es, gas_bus, el_bus, [])

dummy_source_hw = block_creator.get_dummy_source(counter.next(), station_name, 'гвс_источик', hw_bus, 9999)
dummy_source_steam = block_creator.get_dummy_source(counter.next(), station_name, 'пар_источник', steam_bus, 9999)
...
df_dummy_hw = get_dataframe_by_output_bus_single(results, dummy_source_hw, hw_bus) 
...
df_hw_total = df_hw_pt
df_hw_total['Dummy_hw'] = df_dummy_hw
...
ax_hw_pt = df_hw_total.plot(kind="area", ylim=(0, maxY) , ax = third_pos, legend = 'reverse', title = 'Производство гвс' )
ax_gas_pt = df_gas_pt.plot(kind="area", ylim=(0, maxY) , ax = fourth_pos, legend = 'reverse', title = 'Потребление газа' )
ax_eff_pt = df_eff_pt.plot(kind="line", ylim=(0, 1) , ax = fifth_pos, legend = 'reverse', title = 'КПД' )

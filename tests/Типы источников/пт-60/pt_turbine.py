...
chp_pt_60_heat_water_bus = solph.Bus('chp_pt_60_heat_water_bus')
chp_pt_60_steam_bus = solph.Bus('chp_pt_60_steam_bus')
chp_pt_60_electricity_inner_bus = solph.Bus('chp_pt_60_electricity_inner_bus')

energysystem.add(chp_pt_60_heat_water_bus, chp_pt_60_steam_bus, chp_pt_60_electricity_inner_bus)
...
chp_pt_60_T_mode_tr = solph.components.Transformer (
    label = 'chp_pt_60_T_mode_tr',
		inputs = {natural_gas_global_bus: solph.Flow(nominal_value = 198.36)},
		outputs = {heat_water_bus: solph.Flow(),
								chp_pt_60_electricity_inner_bus: solph.Flow()
             },
  	conversion_factors = {natural_gas_global_bus: (1 + 2.02) / 0.91, chp_pt_60_electricity_inner_bus: 1, heat_water_bus: 2.02}
...
chp_pt_60_electricity_main_output_tr = solph.components.Transformer (
    label = 'chp_pt_60_electricity_main_output_tr',
		...
		outputs = {electricity_bus: solph.Flow(nominal_value = 60, min = 0.2, nonconvex = solph.NonConvex()),
             },
 ) 


energysystem.add(chp_pt_60_P_mode_tr, chp_pt_60_T_mode_tr, chp_pt_60_electricity_main_output_tr)
...
electr_df['ПТ-60_электроэнергия'] = electricity_data[((chp_pt_60_electricity_main_output_tr.label, electricity_bus.label),'flow')]
...
gas_df = pd.DataFrame()
gas_df['Расход газа ПТ-60 - П - 1 часть  '] = gas_data[((natural_gas_global_bus.label, chp_pt_60_P_mode_tr.label),'flow')]
gas_df['Расход газа ПТ-60 - П - 2 часть '] = gas_data[((natural_gas_global_bus.label, chp_pt_60_T_mode_tr.label),'flow')]
...
steam_data_df['ТЭЦ-пар-1'] = steam_data[((chp_pt_60_P_mode_tr.label, steam_bus.label),'flow')]

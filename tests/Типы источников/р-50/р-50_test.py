...
from custom_modules.result_proccessing import get_dataframe_by_output_bus
...
##########################################################################################################
[el_bus, gas_bus, steam_bus] = Generic_buses(es).create_buses('электричество_поток','природный_газ_поток', 'пар_поток')
gas_source = Generic_sources(es).create_source('природный_газ_источник', gas_bus, 100)

# print(power_loads)
el_sink = Generic_sinks(es).create_sink_fraction_demand('электричество_потребитель', el_bus, demand_profile=power_loads, peak_load=50)
steam_sink = Generic_sinks(es).create_sink_absolute_demand('пар_потребитель', steam_bus, demand_absolute_data= [450 for _ in range(24)])
...
block_creator.get_dummy_source(counter.next(), station_name, 'пар_источик', steam_bus, 9999)
block_creator.get_p_50(counter.next(), station_name, steam_bus)
...
df_steam = get_dataframe_by_output_bus(results, block_collection, steam_bus)
...
ax_el = df_el.plot(kind="area", ylim=(0, 600), ax = first_pos, legend = 'reverse', title = 'Производство электричества' )
ax_steam = df_steam.plot(kind="area", ylim=(0, 600) , ax = second_pos, legend = 'reverse', title = 'Производство пара' )
